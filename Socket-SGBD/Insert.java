...
public class Insert {
    public static boolean syntaxeFormat(String[] commands, String[][] data) {
        ...
        boolean col = false;
        if (commands[0].compareToIgnoreCase("insert") == 0 && commands[1].compareToIgnoreCase("into") == 0
                && commands[3].compareToIgnoreCase("values") == 0) {
            baseSyntaxe = true;
        }
        if ((commands[4].substring(0, 1).compareTo("(") == 0) && (commands[commands.length - 1]
    ...
    public static String traitement(Server server, String[] instructions) {
        ...
        int d1 = 0;
        int d2 = 0;
        if (new File("../BDD/" + instructions[2] + ".txt").exists()) {
        ...
        if (isDataGet) {
            try {
                ...
                boolean success = Insert.syntaxeFormat(instructions, data);
                try {
                    if (new File("../BDD/" + instructions[2] + ".txt").exists()) {
                        ecrire = new BufferedWriter(new FileWriter(server.getFile(instructions[2]), true));
                        if (success) {
