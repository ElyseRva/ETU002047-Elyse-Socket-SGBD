...
public class Update {
    public static boolean syntaxeFormat(String[] commands) {
        ...
        if (commands[0].compareToIgnoreCase("update") == 0 && commands[2].compareToIgnoreCase("set") == 0)
            baseSyntaxe = true;
        return baseSyntaxe;
    }
    ...
    public static boolean isExist(String[] instructions) {
        ...
        if (new File("../BDD/" + instructions[1] + ".txt").exists()) {
            isExist = true;
        }
        return isExist;
    }

    public static String conditionnel(Server server, String[] instructions, String serverResponse, String tableName,
            boolean condition) {
        String[][] data = null;
        int d1 = 0;
        int d2 = 0;
        ...
        if (colSet != -1 && col != -1) {
            ...
            try {
                ...
                try {
                    ...
                    serverResponse = linesUpdated + " ligne(s) modifiée(s)";
                }
            } catch (IOException e) {
                e.getMessage();
            }
